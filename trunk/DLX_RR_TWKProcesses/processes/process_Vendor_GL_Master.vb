...
Public Class process_Vendor_GL_Master
    ...
    Public Overrides Sub Start()
        ...
        If IO.File.Exists(pFile) = False Then
            ...
            ProcessComplete("Complete", "process_Vendor_GL_Master -> No " & process_file_name & " file found.  Nothing to do")
            Exit Sub
        End If
        Dim pInfo As New IO.FileInfo(pFile)
        Dim namePart As String = pInfo.Name.Replace(pInfo.Extension, "")
        Dim VendorGLMaster_file As New DLX_chess_BLL.file_Vendor_GL_Master(dm_filetypes.idx)
        VendorGLMaster_file.Load(pFile)
        Dim chessDA As New dataacesss_chess
        If chessDA.Skeleton_Records_Create(VendorGLMaster_file, connectionString_Chess, process_sp_name) = dm_SQLResults.Complete Then
        ...
        ProcessComplete("Complete", "process_Vendor_GL_Master -> Vendor GL MASTER update have all completed")
    End Sub
End Class
